...
def ids_dos_melhor_classificados(dados,numero_de_times):
    ...
    while cont < numero_de_times:
        lista_time.append(times[cont])
        cont += 1
    return lista_time
'''
...
def classificados_libertadores(dados):
    num_classif = qtos_libertadores(dados)
    lista_times = ids_dos_melhor_classificados(dados,num_classif)
    return lista_times
...
def nomes_classificados_libertadores(dados):
    ...
    for time in classificados_libertadores(dados):
        nomes_times.append(dados["equipes"][time]["nome-comum"])
    return nomes_times
'''
...
'''
def ids_dos_times_de_um_jogo(dados,id_jogo):
    return dados['fases']['2700']['jogos']['id'][id_jogo]["time1"], dados['fases']['2700']['jogos']['id'][id_jogo]["time2"]
'''
...
def nomes_dos_times_de_um_jogo(dados,id_jogo):
    lista_nomes = []
    lista_time_jogo = ids_dos_times_de_um_jogo(dados, id_jogo)
    for id_time in lista_time_jogo:
        lista_nomes.append(nome_do_time(dados, id_time))
    return lista_nomes
...
def id_do_time(dados,nome_time):
    lista_ids_equipes = dados['equipes']
    for id_time in lista_ids_equipes:
        if lista_ids_equipes[id_time]["nome-comum"] == nome_time:
            return id_time
...
def dicionario_id_estadio_e_nro_jogos(dados):
    ...
    for jogo in id_jogos:
        if id_jogos[jogo]["estadio_id"] not in dicionario_estadios:
            dicionario_estadios[id_jogos[jogo]["estadio_id"]] = 1
        else:
            dicionario_estadios[id_jogos[jogo]["estadio_id"]] += 1
    return dicionario_estadios
...
def datas_de_jogo(dados):
    datas_jogos = dados["fases"]["2700"]["jogos"]["data"]
    return datas_jogos
'''
...
def data_de_um_jogo(dados, id_jogo):
    if id_jogo not in dados["fases"]["2700"]["jogos"]["id"]:
        ...
        data_jogo_id = dados["fases"]["2700"]["jogos"]["id"][id_jogo]["data"]
        return data_jogo_id
...
def ids_de_jogos_de_um_time(dados,time_id):
    ids_jogos_time = []
    ids_jogos =  dados["fases"]["2700"]["jogos"]["id"]
    for i in ids_jogos:
        if dados["fases"]["2700"]["jogos"]["id"][i]["time1"] == time_id or dados["fases"]["2700"]["jogos"]["id"][i]["time2"] == time_id:
